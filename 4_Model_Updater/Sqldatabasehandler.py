...
class sqlhandler:
    ...
    def SqlQueryExec(self, Query, givecount=False, sqlinput=None, commitdatabase=False):
        ...
        Args:
            ...
            givecount (bool, optional): [if give count is False the return value 0 means the query executes.
    ...
    def SqlQueryExecmany(self, query, sqlinputs=None, commitdatabase=True):
        """[Runs many sql queries at the same time. useful for Updating and Inserting large amounts of data]
    ...
    def get_row_result(self, rownumber=0):
        ...
        Args:
            rownumber (int, optional): [The row result we want to be returned]. Must be 0.
    ...
    def get_all_select_rows(self, Query, sqlinput=None):
        """Tries Read many rows and if failed tries 7 times.
        Args:
            Query (string): [The Sql query to execute]
            sqlinput ([type], optional): [The inputs of the query. Defualt None executes query without an input]. Defaults to None.

        Returns:
            [type]: [-1 if there was an error. if successful all the rows of the query]
        """
        for _ in range(0, 7):  # try 7 times
            try:
                # execute with or without input based on sqlinput
                ...
                # If query execution failed reset the database
                print(f"In get_all_select_rows this happened: {e}")
                time.sleep(1)
                self.__init_db()
        # If there was success in Query we shouldn't have reached this line
        print(
            "Couldn't Excecute"
            + Query
            + " in get_all_select_rows Function in Scraper Class"
        )
        return -1
